...
\title[https://github.com/depot-hep]{}
%\date[\today]{}
\date[April 15, 2021]{}
...
\begin{itemize}
	...
	\item Machine learning (ML) - what is it?
	\item Data
	\item ML pipeline
	\item Linear model
	\item Decision tree
...
\includegraphics[width=0.5\textwidth]{what_is_ml.png}
\only<2->{\includegraphics[width=0.25\textwidth]{approximation.png}}

\only<3->{\begin{itemize}
	\item[\mybullet] Compare your understanding of machine learning now and \\
	one (for example) year from now. This will be your understanding.
\end{itemize}}
...
\begin{itemize}
	...
	\footnotesize
	\setbeamertemplate{items}{\mybullet}
	\item In the previous part we explored a family of linear models:
	\begin{itemize}
		\footnotesize
...
%\small
%\vspace{-1em}
%\textcolor{mypurple}{Our (non-linear) life is driven by decision-making, yes/no in a simplest form}
%%\vfill
...
%\begin{figure}
%\centering
%\includegraphics[width=0.4\textwidth]{titanic.png}
%\end{figure}
%
%\vfill
%\centering
%\small
%\begin{textblock*}{.7\paperheight}(0.32\paperwidth, 0.77\paperheight)
%	\textcolor{mypurple}{Intentions to predict things as well}\\
%	\vspace{1ex}
%	\only<2>{\textcolor{myorange}{Can we mathematically formulate this?}}
%\end{textblock*}
%
%%\vfill
%%source: https://ru.wikipedia.org/wiki/%D0%94%D0%B5%D1%80%D0%B5%D0%B2%D0%BE_%D1%80%D0%B5%D1%88%D0%B5%D0%BD%D0%B8%D0%B9
%\end{frame}
...
%\begin{textblock*}{0.9\paperwidth}(0.16\paperwidth, 0.85\paperheight)
%	\footnotesize
%%	\pause
%	\textcolor{Black}{This is a \textbf{greedy algorithm}: locally optimal choice is made at each iteration (split)}
%\end{textblock*}
...
\begin{columns}
	...
	\begin{itemize}
		\footnotesize
		\item<1->[\mybullet] Define $\mathrm{H}(\text{R})$ in a leaf as a goodness of approximation with the "best" constant $c$ 
		\[
		\mathrm{H}(\text{R}) = \min_{c \in \mathbb{Y}}\frac{1}{|N|}\sum_{(\bm{x}_i, y_i) \in \text{R}}\mathcal{L}(y_i,c)
		\]
		\item<2->[\mybullet] or $c_k$ with $\sum_{k}c_k=1$, if tree predicts class probabilities

		\item<3->[\mybullet] Define a fraction of class $k$ in a leaf as $p_k$
%		\item[\mybullet] Define probability of class $k$ in a leaf as $c_k$ ($\sum_{k}c_k = 1$)
		\item<4->[\mybullet] Then one can show:
		\begin{itemize}
			\vspace{.6ex}
			\itemsep0.3ex
			\footnotesize
			\item[\mysubbullet] \textbf{Misclassification criteria}\\$\mathcal{L}(y, c) = [y \ne c] \Rightarrow \mathrm{H}(\text{R})=1-\max _{k}\left\{p_{k}\right\}$ 
			\item[\mysubbullet] \textbf{Gini Impurity}\\$\mathcal{L}(y, c) = \sum_{k}(c_k - [y = k])^2 \Rightarrow  \mathrm{H}(\text{R})=1-\sum_{k}\left(p_{k}\right)^{2}$ 
			\item[\mysubbullet] \textbf{Entropy criteria}\\$\mathcal{L}(y, c) = -\sum_{k}[y = k]\log c_k \Rightarrow \mathrm{H}(\text{R})=-\sum_{k} p_{k} \log _{2} p_{k}$ 
	...
	\begin{figure}
		\centering
		\includegraphics[width=1.\textwidth]{criterion.png}\\
		\vspace{1ex}
		\footnotesize
		\textcolor{black}{\textbf{Lower diversity} (higher purity) \\ $\Rightarrow$ \textbf{lower impurity} criterion H(R)}
	\end{figure}
}
\end{columns}
...
%	\footnotesize
%	\textcolor{Gray}{\href{https://victorzhou.com/blog/gini-impurity/
%		}{\underline{more examples on Gini Impurity}}}
%\end{textblock*}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.75\textwidth]{criterion.eps}
%\end{figure}
%\end{frame}
...
\vspace{0.3\paperheight}
\begin{itemize}
	\item[\mybullet] How should we modify decision criteria?
	\[
	\text{H}(j, t) = 
	\dfrac{|\text{L}|}{|\text{Q}|} \text{H}(\text{L}) + \dfrac{|\text{R}|}{|\text{Q}|}\text{H}(\text{R})
	\]
	\item[\textcolor{myblack}{\MVRightArrow}]  \textcolor<2->{myorange}{Mean squared error:} $\quad \mathcal{L}(y, c) = (y-c)^2 \Rightarrow \text{H}(\text{R})=\min_{c} \frac{1}{|\text{R}|} \sum_{\left(x_{i}, y_{i}\right) \in \text{R}}\left(y_{i}-c\right)^{2}$
	\item[\textcolor{myblack}{\MVRightArrow}] \textcolor<2->{mypurple}{Mean absolute error:} $\quad \mathcal{L}(y, c) = |y-c| \Rightarrow \text{H}(\text{R})=\min_{c} \frac{1}{|\text{R}|} \sum_{\left(x_{i}, y_{i}\right) \in \text{R}}|y_{i}-c|$
	\item<2->[\mybullet] One can further show that:
	\textcolor<2->{myorange}{\[
	c^{*}=\frac{1}{|\text{R}|} \sum_{y_{i} \in \text{R}} y_{i}
	\]}
	\textcolor<2->{mypurple}{\[
	c^{*}= \text{median}(y_{i} \in \text{R})
	\]}
\end{itemize}
...
%\begin{figure}
%\centering
%\includegraphics[width=.75\textwidth]{dregr.eps}
%\end{figure}
%\end{frame}
...
\subsection{features in ML: not HEP}
